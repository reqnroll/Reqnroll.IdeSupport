...
into `src/VSCode/mtpreporter/`. Like the TestLogger, there's no RID to choose. What the extension
uses from it is the reporter's *source bundle* (issue #741): `Reqnroll.IdeSupport.TestReporter.MTP.targets`
plus `ReporterSource/*.cs`. At activation the extension writes a project-local

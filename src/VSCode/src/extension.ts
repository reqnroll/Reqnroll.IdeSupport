...
import { createReqnrollTestController } from './testRunner/reqnrollTestController';
import {
  ManualDocumentSync,
...
export function activate(context: vscode.ExtensionContext): ReqnrollExtensionApi {
  ...
  client
    ...
    .then(() => {
      ...
      // Native Testing-sidebar presence for .feature scenarios (issue #262, migrated off the
      // CodeLens-only "own execution" design by issue #504) — Run/Debug, pass/fail history, and
      // the failed-step marker are all native vscode.TestController/TestRun behavior now.
      createReqnrollTestController(client!, context, projectManager);

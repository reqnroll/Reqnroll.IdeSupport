...
/// <summary>
/// Regression guard for <see cref="ProjectSystemIdeSupportConfigurationProvider"/>'s known,
/// still-open no-op: solution-level configuration is never loaded, so
/// <see cref="ProjectSystemIdeSupportConfigurationProvider.GetConfiguration"/> always returns a
/// fresh default <see cref="IdeSupportConfiguration"/>, and
/// <see cref="ProjectSystemIdeSupportConfigurationProvider.ConfigurationChanged"/> is never raised.
...
/// </summary>
public class ProjectSystemIdeSupportConfigurationProviderTests
{
    [Fact]
    public void GetConfiguration_returns_a_default_configuration_regardless_of_ide_scope()
    {
        ...
        var provider = new ProjectSystemIdeSupportConfigurationProvider(ideScope);
    ...
    {
        var provider = new ProjectSystemIdeSupportConfigurationProvider(Substitute.For<IIdeScope>());

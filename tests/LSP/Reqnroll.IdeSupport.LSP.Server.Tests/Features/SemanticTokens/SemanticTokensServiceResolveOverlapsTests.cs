...
/// <summary>
/// Unit tests for <see cref="SemanticTokensService.ResolveOverlaps"/>, the token-overlap-splitting
...
public class SemanticTokenServiceResolveOverlapsTests
{
    ...
    public void No_overlap_returns_entries_unchanged()
    {
        ...
        var resolved = SemanticTokensService.ResolveOverlaps(entries);
    ...
    {
        SemanticTokensService.ResolveOverlaps(new List<(int, int, int, int, int)>()).Should().BeEmpty();
    }
}

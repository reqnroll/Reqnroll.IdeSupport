...
public class ConnectorLogPathsTests
{
    [Fact]
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), $"reqnroll-connector-prune-{Guid.NewGuid():N}");
        Directory.CreateDirectory(dir);
        try
        {
            var stale = Path.Combine(dir, "reqnroll-lsp-connector-20200101-1.log");
            var fresh = Path.Combine(dir, "reqnroll-lsp-connector-20990101-2.log");
            ...
            ConnectorLogPaths.PruneOldLogFiles(dir);
    ...
    {
        var act = () => ConnectorLogPaths.PruneOldLogFiles(
